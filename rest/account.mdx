...
Manually reset MMP after it has been triggered, restoring quoting activity.

```typescript
await client.resetMMP({
  baseCoin: 'BTC',
});
```
...
Query the current MMP configuration and state for a base coin.

```typescript
const mmpState = await client.getMMPState({
  baseCoin: 'BTC',
});
